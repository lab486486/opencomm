# -*- coding: utf-8 -*-
"""드라마 자동 포스팅 시스템 메인 실행 스크립트 (DeepSeek V4 / Gemini)."""
...
import config
from drama_deepseek import DeepSeekDramaGenerator
from drama_gemini import GeminiDramaGenerator
from drama_instruction import parse_drama_instruction
...
def process_single_drama(
    ...
    logger.info("━━━ 드라마 지시글 처리 시작: ID=%s ━━━", post_id)

    try:
        instruction = parse_drama_instruction(post)
        logger.info(
            "파싱 결과: 드라마='%s', 썸네일='%s', 유튜브='%s'",
            instruction.drama_title,
            instruction.thumbnail_url or "없음",
            instruction.youtube_url or "없음",
        )
        ...
        if instruction.thumbnail_url:
            try:
                featured_media_id = wp_client.upload_media_from_url(
                    instruction.thumbnail_url,
                    title=f"{instruction.drama_title} 대표 이미지",
                )
                logger.info("대표 썸네일 등록 성공: ID=%s", featured_media_id)
            except Exception as e:
                logger.warning("썸네일 업로드 실패 (계속 진행): %s", e)

        if dry_run:
            logger.info("[DRY-RUN] 발행 생략")
            logger.info("[DRY-RUN] 제목: %s", generated.title)
            logger.info("[DRY-RUN] 슬러그: %s", generated.slug)
            logger.info("[DRY-RUN] 태그: %s", generated.tags)
            logger.info("[DRY-RUN] 본문 길이: %s자", len(html_content))
            return True

        new_post = wp_client.publish_post(
            title=instruction.source_title,
        ...
        logger.info("━━━ 드라마 지시글 처리 완료: ID=%s ━━━", post_id)
        return True

    except Exception as exc:
        logger.exception("지시글 처리 실패: ID=%s, error=%s", post_id, exc)
        if not dry_run:
...
def main(args: argparse.Namespace) -> None:
    setup_logging(
        ...
        None if args.no_log_file else config.LOG_FILE,
    )
    logger.info("=" * 70)
    logger.info("드라마 자동 포스팅 시스템 시작: %s", datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="WordPress + DeepSeek/Gemini 드라마 자동 포스팅")
    ...
    parser.add_argument("--status", choices=["publish", "draft", "pending", "private"], default=None)
    parser.add_argument("--action", choices=["tag", "tag_draft", "trash", "delete"], default=None)
    parser.add_argument("--model", type=str, default=None, help="모델명")
    parser.add_argument("--force-peak", action="store_true", help="피크타임이어도 API 강제 호출")
    parser.add_argument("--instruction-category", type=str, default=None)
